...
class EmberTokensManager {
    ...
    static async saveTokens(ezsp, localEui64) {
        ...
        if (tokenCount) {
                ...
                if (tiStatus === enums_1.EmberStatus.SUCCESS) {
                    ...
                    for (let arrayIndex = 0; arrayIndex < tokenInfo.arraySize; arrayIndex++) {
                        ...
                        if (tdStatus === enums_1.EmberStatus.SUCCESS) {
                            ...
                            logger_1.logger.error(`[TOKENS] Failed to get token data at index ${arrayIndex} with status=${enums_1.EmberStatus[tdStatus]}.`, NS);
                        }
                    }
                    chunks.push(outputToken);
                }
                else {
                    logger_1.logger.error(`[TOKENS] Failed to get token info at index ${i} with status=${enums_1.EmberStatus[tiStatus]}.`, NS);
                }
            }
            ...
            logger_1.logger.error(`[TOKENS] Saving tokens not supported by NCP (not NVM3-based).`, NS);
        }
        return null;
    }
    /**
            ...
            if (tiStatus === enums_1.EmberStatus.SUCCESS) {
                ...
                for (let arrayIndex = 0; arrayIndex < arraySize; arrayIndex++) {
                    ...
                    if (status !== enums_1.EmberStatus.SUCCESS) {
                        logger_1.logger.error(`[TOKENS] Failed to set token data for key "${nvm3Key}" with status=${enums_1.EmberStatus[status]}.`, NS);
                    }
    ...
    static async writeNcpTokensToZigbeedTokens(ezsp, inBuffer) {
        ...
        for (let i = 0; i < inTokenCount; i++) {
            ...
            for (let arrayIndex = 0; arrayIndex < arraySize; arrayIndex++) {
                ...
                if (status !== enums_1.EmberStatus.SUCCESS) {
                    logger_1.logger.error(`[TOKENS] Failed to set Zigbeed token data for key "${nvm3Key}" creator "${creator}" with status=${enums_1.EmberStatus[status]}.`, NS);
                }
                readOffset += tokenData.size;
            }
        }
