...
class Driver extends events_1.default.EventEmitter {
        ...
        setInterval(() => {
            that.writeParameterRequest(0x26, 600) // reset watchdog // 10 minutes
                ...
                that.writeParameterRequest(0x26, 600)
                    .then(result => { })
                    .catch(error => { logger_1.logger.debug("warning watchdog was not reset", NS); });
            });
        }, (1000 * 60 * 8)); // 8 minutes
    ...
    catchPromise(val) {
        return Promise.resolve(val)
            .catch(err => logger_1.logger.debug(`Promise was caught with reason: ${err}`, NS));
    }
    setDelay(delay) {
    ...
    async checkDeviceStatus(currentDeviceStatus) {
        ...
        logger_1.logger.debug("networkstate: " + networkState + " apsDataConfirm: " + this.apsDataConfirm + " apsDataIndication: " + this.apsDataIndication +
